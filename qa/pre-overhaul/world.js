...
export class TronWorld {
  constructor(scene) {
    ...
    // 8x8 districts of 620 = 4960 across, plus breathing room at the edge.
    this.gridSize = 4960;
  ...
  // ==================================================================
  initGrid() {
    const gridMajor = new THREE.GridHelper(this.gridSize, 248, 0x35f6ff, 0x11405e);
    gridMajor.position.y = 0.02;
    this.scene.add(gridMajor);
    this.gridMesh = gridMajor;

    const planeGeo = new THREE.PlaneGeometry(this.gridSize * 1.18, this.gridSize * 1.18);
  ...
  districtCenter(dx, dz) {
    // centred for an 8-wide grid: dx 0..7 -> -3.5..3.5 districts
    return { x: (dx - 3.5) * DISTRICT, z: (dz - 3.5) * DISTRICT };
  }

  /** 3x3 sector grid: each sector owns 2x2 districts and its own palette. */
  sectorFor(pos) {
    const half = DISTRICT;                     // 2 districts per sector = 2*620
    // The indexed Grid is 3x3 sectors centred on the origin: x=0 must land in
    // sector 1,1 (CORE GRID), not on a boundary between sectors.
    if (Math.abs(pos.x) > half * 3 || Math.abs(pos.z) > half * 3) {
      return '3,3';                      // the OUTLANDS ring has its own palette
    }
  ...
  initDistricts() {
    ...
    // 8x8. The inner 6x6 is the indexed Grid; ring 0 and ring 7 are the
    // OUTLANDS — the unsealed frontier the campaign opens checkpoint by
    // checkpoint (13 plains, 14 pylon field, 15 rift ruins, 16 tower approach).
    const layout = [
    ...
    for (let dz = 0; dz < 8; dz++) {
      for (let dx = 0; dx < 8; dx++) {
        ...
        switch (type) {
          ...
          case 13: this.buildOutlandPlains(c); break;
          case 14: this.buildPylonField(c); break;
          case 15: this.buildRiftRuins(c); break;
          case 16: this.buildTowerApproach(c); break;
  ...
  // ------------------------------------------------------------------
  //  OUTLANDS — the frontier beyond the indexed Grid.
  // ------------------------------------------------------------------

  /** 13 — OUTLAND PLAINS: sparse derelict shells and dead cabling. */
  buildOutlandPlains(c) {
    for (let i = 0; i < 12; i++) {
      const x = c.x + this.rr(-270, 270);
      const z = c.z + this.rr(-270, 270);
      const h = this.rr(6, 24);
      this._pushBox(x, h / 2, z, this.rr(12, 34), h, this.rr(12, 34), 0xff3b6b, { ribs: 1 });
    }
    for (let i = 0; i < 5; i++) {
      const x = c.x + this.rr(-250, 250);
      const z = c.z + this.rr(-250, 250);
      this.addCollider(x - 2, x + 2, z - 2, z + 2, 0, 34);
      this._sym(0xff6a8a, x, 17, z, 1.2, 34, 1.2, 0xff3b6b);
    }
  }

  /** 14 — PYLON FIELD: beacon masts marking the sealed frontier. */
  buildPylonField(c) {
    for (let i = 0; i < 9; i++) {
      const x = c.x + this.rr(-250, 250);
      const z = c.z + this.rr(-250, 250);
      const h = this.rr(60, 150);
      this.addCollider(x - 4, x + 4, z - 4, z + 4, 0, h);
      this._sym(0xb46bff, x, h / 2, z, 3, h, 3, 0xb46bff);
      const cap = new THREE.Mesh(
        new THREE.TorusGeometry(14, 1.1, 6, 20),
        new THREE.MeshBasicMaterial({ color: 0xd9b0ff })
      );
  ...
  /** 15 — RIFT RUINS: where the crossing happened. It never healed. */
  buildRiftRuins(c) {
    ...
    for (let i = 0; i < 16; i++) {
      ...
      const r = 150 + this.rr(0, 90);
      const x = c.x + Math.cos(a) * r;
      const z = c.z + Math.sin(a) * r;
      const h = this.rr(14, 52);
      this._pushBox(x, h / 2, z, this.rr(14, 30), h, this.rr(14, 30), 0xb46bff, { ribs: 2 });
    }
  }

  /** 16 — TOWER APPROACH: the I/O Tower, and everything guarding it. */
  buildTowerApproach(c) {
    const H = 420;
    ...
    // guard blocks around the base
    for (let i = 0; i < 14; i++) {
      const a = (i / 14) * Math.PI * 2;
      const r = 120 + this.rr(0, 130);
      const x = c.x + Math.cos(a) * r;
      const z = c.z + Math.sin(a) * r;
      const h = this.rr(18, 60);
      this._pushBox(x, h / 2, z, this.rr(16, 34), h, this.rr(16, 34), 0x00f0ff, { ribs: 2 });
    }
  }
  ...
  updateRegion(delta, playerPos) {
    ...
    if (key !== this._sectorKey) {
      ...
      this._to = TronWorld.SECTORS[key] || TronWorld.SECTORS['3,3'];
      this._blend = 0;
